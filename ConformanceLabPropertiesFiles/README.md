...
| Conformancelab property | input                                                    | required? | override possible?   | notes                                      |
| ----------------------- | -------------------------------------------------------- | --------- | -------------------- |------------------------------------------- |
| `fhirVersion`           | ANT property `fhir.version`                              | x         | x                     | Must be either "STU3" or "R4". Also used in other build steps.|
| `goal`                  | ANT `goal` property, or root folder name                 |           | x                     |                                            |
| `informationStandard`   | ANT property `informationStandard`                       | x         | x                     |                                            |
| `usecase`               | ANT property `usecase`                                   | x         | x                     |                                            |
| `category`              | first subfolder which is not role                        |           | x                     |                                            |
| `subcategory`           | second subfolder which is not role                       |           | x                     |                                            |
| `role`                  |                                                          | x         | x                     |                                            |
| - `name`                | subfolder which matches entry in ANT property `roles`    | x         | x                     | `roles` is a comma separated list. See also `variant`|
| - `description`         | ANT property `role.description.`_role_                   | x         | x                     |                                            |
| `variant`               |                                                          |           | x                     |                                            |
| - `name`                | Addition to `role` in the folder name                    |           | x                     | A variant will be emitted of the role folder contains an addition |
| - `description`         | ANT property `target.description`._target_               | (x)       | x                     | e.g. `target.description.XIS-Server-Nictiz-only = For Nictiz only |
| `adminOnly`             | ANT property `targets.adminOnly`                         | (x)       | x                     | Either "true" or "false"                                          |
| `fhirPackage`           |                                                          |           |                      |                                            |
| - `name`                | ANT property `packages`                                  |           | x                     | This is a comma separated list of package canonicals              |
| - `version`             | ANT property `package.`_canonical_                       |           | x                     | e.g. `package.nictiz.stu3.zib2017 = 2.2.3`                        |
| `serverAlias`           | ANT property `server`, or                                | (x)       |                      | One of these must be provided                                     |
|                         | match on ANT property `server.`_usecase_`.`_fhirVersion_ | (x)       | x                     | One of these must be provided                                     |
...
Overriding the `fhirPackage` and `serverAlias` properties is not supported.