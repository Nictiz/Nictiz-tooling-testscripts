                ...
                <test id="{@id}-content">
                                        ...
                                        <xsl:otherwise>
                                            <xsl:for-each select="nts:expressionPart">
                                                ...
                                                <xsl:choose>
                                                    ...
                                                    <xsl:when test="count(ancestor::nts:generateAsserts/nts:resource[@type = $resourceType]) = count(ancestor::nts:generateAsserts/nts:resource[@type = $resourceType]/nts:*[text()=$contents])"/>
                                                    <xsl:otherwise>
                                                        <xsl:copy-of select="."/>
                                                    </xsl:otherwise>
                                                </xsl:choose>
                    ...
                    <xsl:for-each select="$generateCombinedExpression/nts:generateAsserts/nts:resource">
                        ...
                        <xsl:variable name="is-unique">
                            <xsl:variable name="contents" select="text()"/>
                            <xsl:choose>
                                <xsl:when test="count($generateCombinedExpression/nts:generateAsserts/nts:resource[text() = $contents]) = 1 or count($generateCombinedExpression/nts:generateAsserts/nts:resource[@type = $resourceType]) = 1">
                                    <xsl:value-of select="true()"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of select="false()"/>
                        ...
                        <xsl:variable name="position" select="count(preceding-sibling::nts:resource) + 1"/>
                        ...
                        <xsl:choose>
                            <xsl:when test="$is-unique = true()">
                                ...
                                <xsl:if test="contains(text(), '${DATE, T') and $TestScriptHasT = false() and not(preceding-sibling::nts:resource[contains(text(), '${DATE, T')])">
                                    <variable>
                                        <name value="T"/>
                                ...
                                <xsl:variable name="resourceExpression">
                                    ...
                                    <xsl:text>)</xsl:text>
                                    <xsl:if test="count($generateCombinedExpression/nts:generateAsserts/nts:resource[@type = $resourceType]) gt 1">
                                ...
                                <xsl:for-each select="$fixturesResources/*[$position]">
                                    ...
                                    <xsl:apply-templates select="." mode="asserts">
                                        ...
                                        <xsl:with-param name="has-T" as="xs:boolean" tunnel="yes">
                                            <xsl:choose>
                                                <xsl:when test="$TestScriptHasT = true() or $generateCombinedExpression/nts:generateAsserts/nts:resource[contains(text(), '${DATE, T')]">
                                                    <xsl:value-of select="true()"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="false()"/>
